...
if __name__ == "__main__":
    ...
    def make_env():
        env = Origin_custom_envs.PowerSystemEnv(mpc, path)
    ...
    writer = SummaryWriter("runs/30bus_11")
    writer.add_text(
        "hyperparameters",
    ...
    # V_last = None
    # adjust = np.full((grid_data['num_gen']), 0.03)
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            # #在优化时期对actions的V进行约束，防止因为V的过宽调整导致Q越界
            # if V_last is not None:
            #     actions[0, grid_data['num_non_slack']:grid_data['num_non_slack']+grid_data['num_gen']] = np.clip(
            #         V_last,
            #         V_last - adjust,
            #         V_last + adjust
            #     )
            # # print(f'actions中的V是:{actions[0, 5:11]}')
            # V_last = actions[0, grid_data['num_non_slack']:grid_data['num_non_slack']+grid_data['num_gen']]
