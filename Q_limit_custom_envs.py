...
import Q_limit_test
from Q_limit_test import check_Q_limit
from Q_limit_test import Translate_PV_to_PQ_HE
import warnings
warnings.filterwarnings("ignore", category=FutureWarning)

class Q_limit_PowerSystemEnv(gym.Env):
    def __init__(self, mpc, path):
        ...
        self.net_HE = my_HELMpy.HELM(net)
    ...
    def test_calculate_dcv(self, state):
        ...
        in_boundary = np.where(q_viol_non_slack <= 0.01)[0]  #这里直接在non_slack中选择,不考虑将ref改变成PQ节点
    ...
    def step(self, action):
        ...
        p_d_curr = actual_load_p
        q_d_curr = actual_load_q
        if self.grid['num_wind'] > 0:
        ...
        #检查non_slack_gen的Q越限问题
        his_over_limit_indices = []
        max_iter = 7
        iter = 0
        flag, info_dcv = check_Q_limit(self, self.current_state)
        if flag:
            while(flag):
                # print(f"PV越限,越限机组是{info_dcv['over_limit_indices']}")
                V_HE, S_HE, terms, state = Translate_PV_to_PQ_HE(self, action, info_dcv, his_over_limit_indices)

                iter += 1
                his_over_limit_indices = list(set(his_over_limit_indices) | set(info_dcv['over_limit_indices']))
                # print(f"下次传进去的his是:{his_over_limit_indices}")

                flag, info_dcv = check_Q_limit(self, state)
                if iter == max_iter:
                    break
            #迭代完成后消除了Q带来的dcv,这时候处理真正的state
